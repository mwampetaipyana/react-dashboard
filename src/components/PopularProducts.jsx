            ...
            {popularProductsData.map( product => (
                <Link to={`/product/${product.id}`} className='flex hover:no-underline'>
                  ...
                  <div className='ml-4 flex-1 '>
                    ...
                    <span className={`text-sm font-medium ${product.product_stock === 0 ? 'text-orange-500':
                    'text-green-500' }`}>{product.product_stock === 0 ? 'Out of Stock' :
                   product.product_stock + 'in stock' }</span>
                  </div>
